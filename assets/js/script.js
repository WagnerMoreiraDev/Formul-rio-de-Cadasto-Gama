// função para verificar preenchimento de campos obrigatórios
function checarCamposObrigatorios() {

    // Criado um array para armazenar os dados
    let camposObrigatorios = [];

    // Se o input nome estiver vazio, o array recebe "Nome"
    if (document.getElementById("nome").value == "") {
        camposObrigatorios.push(" Nome");
    }

    // Se o input sobrenome estiver vazio, o array recebe "Sobrenome"
    if (document.getElementById("sobrenome").value == "") {
        camposObrigatorios.push(" Sobrenome");
    }

    // Se o input cpf estiver vazio, o array recebe "CPF"
    if (document.getElementById("cpf").value == "") {
        camposObrigatorios.push(" CPF");
    }

    // Se o input email estiver vazio, o array recebe "E-mail"
    if (document.getElementById("email").value == "") {
        camposObrigatorios.push(" E-mail");
    }

    // Se o input celular estiver vazio, o array recebe "Telefone celular"
    if (document.getElementById("celular").value == "") {
        camposObrigatorios.push(" Telefone celular");
    }

    // Se o input cep estiver vazio, o array recebe "CEP"
    if (document.getElementById("cep").value == "") {
        camposObrigatorios.push(" CEP");
    }

    // Se o input logradouro estiver vazio, o array recebe "Logradouro"
    if (document.getElementById("logradouro").value == "") {
        camposObrigatorios.push(" Logradouro");
    }

    // Se o input numero estiver vazio, o array recebe "Nº"
    if (document.getElementById("numero").value == "") {
        camposObrigatorios.push(" Nº");
    }

    // Se o input bairro estiver vazio, o array recebe "Bairro"
    if (document.getElementById("bairro").value == "") {
        camposObrigatorios.push(" Bairro");
    }

    // Se o input cidade estiver vazio, o array recebe "Cidade"
    if (document.getElementById("cidade").value == "") {
        camposObrigatorios.push(" Cidade");
    }

    // Se o input estado estiver vazio, o array recebe "Estado"
    if (document.getElementById("estado").value == "") {
        camposObrigatorios.push(" Estado");
    }

    // Emite um alerta de campos obrigatórios
    if (camposObrigatorios.length == 1) { // Caso o array contenha apenas uma posição

        // Alerta emitido no singular com o único item do array
        alert("Atenção! O campo" + camposObrigatorios + " é obrigatório e não foi preenchido.");
        camposObrigatorios = []; // Limpa o array
    } else if (camposObrigatorios.length > 1) { // caso o array contenha dois valores ou mais

        // Emite um alerta de campos obrigatórios no plural com todos os itens do array separando o penúltimo e o último pela conjunção "e"
        alert("Atenção! Os campos" + camposObrigatorios.slice(0, (camposObrigatorios.length - 1)) + " e" + camposObrigatorios.slice(-1) + " são obrigatórios e não foram preenchidos.");
        camposObrigatorios = []; // Limpa o array
    } else { // Caso nenhum campo obrigatório esteja em branco, libera o formulário para envio
...
// Fetch API enviando dados do formulário para WebHook.site
const formulario = document.getElementById("formulario");

formulario.addEventListener('submit', function(cadastrar) {
    cadastrar.preventDefault()

    // Criadas as variáveis para armazenar os valores a serem enviados
    let nome = document.getElementById("nome").value;
    let sobrenome = document.getElementById("sobrenome").value;
    let cpf = document.getElementById("cpf").value;
    let rg = document.getElementById("rg").value;
    let sexo; // O valor da variável sexo vai depender do que foi selecionado no input radio
    if (document.getElementById("feminino").checked) {
        sexo = "Feminino";
    } else if (document.getElementById("masculino").checked) {
        sexo = "Masculino";
    } else if ((document.getElementById("prefiro-nao-informar").checked)) {
    ...
    let privacidade; // A variável privacidade só receberá os dados caso a checkbox seja marcada
    if (document.getElementById("privacidade").checked == true) {
        privacidade = "Concordo em compartilhar os meus dados";
    } else {
        privacidade = "";
    }

    // Faz o envio dos dados
    fetch('https://webhook.site/115c05a4-ad32-4e56-a6e5-868e4ced21a7 ', {
        method: 'POST',
        body: JSON.stringify({
            nome,
            sobrenome,
    ...
    // Emite um alerta de envio personalizado com o nome preenchido no formulário
    alert("Parabéns, " + nome + "! Seus dados foram cadastrados com sucesso.");
...
const checkbox = document.querySelector("#privacidade");
const botaoCadastrar = document.querySelector("#cadastrar");
checkbox.addEventListener("change", (e) => botaoCadastrar.disabled = !e.target.checked);
